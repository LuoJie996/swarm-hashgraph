...
from swirld_object_searching2 import Node


R_COLORS = small_palettes['Greens'][9]
shuffle(list(R_COLORS))
def round_color(r):
    return R_COLORS[r % 9]
...
class App:
    def __init__(self, n_nodes):
        ...
        self.nodes = [Node(self.ids[kp[0]], kp, network, n_nodes, stake) for kp in kps]
        for n in self.nodes:
        ...
        def toggle():
            if play.label == '► Play':
                play.label = '❚❚ Pause'
                self.callback_obj = curdoc().add_periodic_callback(self.animate, 50)
            else:
                play.label = '► Play'
                curdoc().remove_periodic_callback(self.callback_obj)
